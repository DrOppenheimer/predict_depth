...
then
    ...
    echo 
    echo "USAGE: > process_LCA_counts.sh <file_in> <include_ambiguous_counts> <tax_level>"
    echo   
    ...
    echo "           process_LCA_counts.sh LCA_test_data.txt TRUE order"
    echo
...
echo "process_LCA_counts(abundance_matrix=\"$file_in\", include_ambiguous_counts=$include_ambiguous_counts, tax_level=\"$tax_level\")" >>  shell.process_LCA_counts.r.$time_stamp.r
